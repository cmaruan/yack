...
static void
x86_gdt_set(struct gdt_entry *entry, u32 base, u32 limit, u8 access, u8 flags)
{
        // base
...
segments_init()
{
        ...
        x86_gdt_set(&cpu->cpu_gdt[X86_GDT_SEG_DATAU], 0, 0xFFFFFFFF, 0xF2,
                    0xCF);
        x86_gdt_set(&cpu->cpu_gdt[X86_GDT_SEG_TSS], (u32)TSS, sizeof(TSS), 0xF2,
                    0xCF);
...
vm_map_page_(pde_t *directory, void *vaddr, void *paddr, u16 permissions)
{
        ...
        page_table[table_index] = (u32)paddr | permissions | X86_PTE_PRESENT;
        return 0;
}